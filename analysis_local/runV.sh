...
if [ ${initMode} == "S" ]; then # Only overwrite initMode and simulT in inpar if restarting from previous simulation
    ...
    if [ "${couplingMode}" == "C" ]; then
        ...
        sed -i "18s/.*/${savePos}/" 'inpar'
        sed -i "19s/.*/${saveInitPos}/" 'inpar'
        sed -i "21s/.*/${saveCoupling}/" 'inpar'
    elif [ "${couplingMode}" == "G" ]; then
        sed -i "13s/.*/${DT}/" 'inpar'
        sed -i "14s/.*/${DTex}/" 'inpar'
        sed -i "15s/.*/${eqT}/" 'inpar'
        sed -i "16s/.*/${simulT}/" 'inpar'
        sed -i "17s/.*/${savePos}/" 'inpar'
        sed -i "18s/.*/${saveInitPos}/" 'inpar'
        sed -i "20s/.*/${saveCoupling}/" 'inpar'
    elif [ "${couplingMode}" == "F" ]; then
        sed -i "13s/.*/${DT}/" 'inpar'
        sed -i "14s/.*/${DTex}/" 'inpar'
        sed -i "15s/.*/${eqT}/" 'inpar'
