...
def plot_vorder_ksd(mode, nPart, phi, KAVG, KSTD_range, seed_range, avg_over, log=False):
    """
    ...
    for KSTD in KSTD_range:
        ...
        for seed in seed_range:
            v_ss_sum += v_order_ss(mode="G", nPart=nPart, phi=phi, K=str(KAVG) + "_" + str(KSTD), seed=seed, avg_over=avg_over)
    ...
    plt.savefig(os.path.abspath('../plots/v_order_vs_K/' + mode + '_N' + str(nPart) + '_phi' + str(phi) + '_KAVG' + str(KAVG) + '.png'))


def plot_vorder_kavg(mode, nPart, phi, KAVG_range, KSTD, seed_range, avg_over):
    """
    ...
    plt.savefig(os.path.abspath('../plots/v_order_vs_K/' + mode + '_N' + str(nPart) + '_phi' + str(phi) + '_KSTD' + str(KSTD) + '.png'))


def plot_vorder_kavg_superimpose(mode, nPart_range, phi, KAVG_range, KSTD, seed, avg_over):
    fig, ax = plt.subplots()
    for nPart in nPart_range:
        v_ss = []
        for KAVG in KAVG_range:
            v_ss.append(v_order_ss(mode="G", nPart=nPart, phi=phi, K=str(KAVG) + "_" + str(KSTD), seed=seed, avg_over=avg_over))
        ax.plot(KAVG_range, v_ss, 'o-', label="N=" + str(nPart))
    ax.set_xlabel("KAVG")
    ax.set_ylabel(r"Vicsek order parameter, $\Psi$")
    ax.legend()

    plt.savefig(os.path.abspath('../plots/v_order_vs_K/' + mode + '_phi' + str(phi) + '_KSTD' + str(KSTD) + '_s' + str(seed) + '.png'))
...
def v_order_sd(mode, nPart, phi, K, seed, avg_over):
    ...
    for theta_t in theta_all:
        ...
        v_order.append(np.sqrt(cos_sum**2+sin_sum**2)/nPart)
    v_sd = np.std(v_order)
    # return v_sd               # standard deviation
    return nPart*v_sd**2        # susceptibility


def plot_vorder_sd_ksd(mode, nPart, phi, KAVG, KSTD_range, seed_range, avg_over):
    """
    Plot standard deviation of Vicsek order parameter against K_std for a given K_avg
    Averaged over a number of realizations
    """
    v_sd = []
    for KSTD in KSTD_range:
        v_sd_sum = 0
        for seed in seed_range:
            v_sd_sum += v_order_sd(mode="G", nPart=nPart, phi=phi, K=str(KAVG) + "_" + str(KSTD), seed=seed, avg_over=avg_over)
        v_sd.append(v_sd_sum/len(seed_range))
    fig, ax = plt.subplots()
    ...
    plt.savefig(os.path.abspath('../plots/v_order_sd_vs_K/' + mode + '_N' + str(nPart) + '_phi' + str(phi) + '_KAVG' + str(KAVG) + '.png'))
        

def plot_vorder_sd_kavg(mode, nPart, phi, KAVG_range, KSTD, seed_range, avg_over):
    """
    Plot the standard deviation of the Vicsek order parameter (over the final 1000 saved timesteps) against K_avg, with a fixed K_std
    Averaged over a number of realizations
    """
    v_sd = []
    for KAVG in KAVG_range:
        ...
        v_sd.append(v_sd_sum/len(seed_range))
    fig, ax = plt.subplots()
    ax.plot(KAVG_range, v_sd, 'o-')
    ax.set_xlabel("KAVG")
    ax.set_ylabel(r"Vicsek order parameter SD")
    
    plt.savefig(os.path.abspath('../plots/v_order_sd_vs_K/' + mode + '_N' + str(nPart) + '_phi' + str(phi) + '_KSTD' + str(KSTD) + '.png'))

def plot_vorder_N(mode, nPart_range, phi, K, seed, avg_over, log=False):
    """
    Plot the mean Vicsek order parameter (averaged over the final 1000 saved timesteps) against the number of particles in the system N, 
    for a given K (or "K_avg_K_std")
    """
    v_ss = []
    for nPart in nPart_range:
        v_ss.append(v_order_ss(mode=mode, nPart=nPart, phi=phi, K=K, seed=seed, avg_over=avg_over))
    fig, ax = plt.subplots()
    ...
    if log == True:
        ax.set_xscale("log")
    plt.savefig(os.path.abspath('../plots/v_order_vs_N/' + mode + '_phi' + str(phi) + '_K' + str(K) + '_s' + str(seed) + '.png'))
