           ...
           vector<double>& ffx, vector<double>& ffy, vector<double>& ffp)
{
    ...
    for (int i=0 ; i<nPart ; i++) {
        ...
        for (int j=cl[i] ; j<cl[i+1] ; j++) {
            ...
            if (xij - Lx*rint(xij/Lx) == 0) {
                cout << "Error loc 2.1" << endl;
                cout << i << endl;
                cout << xij << '\t' << Lx << endl;
                cout << rint(xij/Lx) << endl;
                cout << xx[i] << '\t' << xx[nl[j]] << endl;
                // ::exit(1);
            }
            ...
            if (fabs(xij) <= rc) { // rc = MAX(rr,rp)
                ...
                if (rijsq <= rrsq) {
                    ...
                    // if (isnan(ff)) {
                    if (ff < -1e5) {
                        cout << "Error loc 2" << endl;
                        cout << ff << endl;
                        cout << gx << endl;
                        cout << xij << '\t' << yij << endl;
                        cout << rijsq << '\t' << rij << endl;
                        cout << xij << '\t' << xx[i] << '\t' << xx[nl[j]] << endl;
                        cout << i << '\t' << j << '\t' << nl[j] << endl;
                        ::exit(1);
                    }
