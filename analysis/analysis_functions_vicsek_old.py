...
def plot_porder_Kavg(mode, nPart_range, phi, noise_range, K_avg_range, K_std_range, xTy, seed_range, save_data=False):
    """
    ...
    for nPart in nPart_range:
        for noise in noise_range:
            for K_std in K_std_range:
                ...
                if save_data == True:
                    save_file.write(str(nPart) + "\t" + str(phi) + "\t" + str(noise) + "\t" + str(K_std) + "\n")
                    for K_avg in K_avg_range:
    ...
    filename = mode + '_N' + str(nPart) + '_phi' + str(phi) + '_n' + str(noise) + '_Kstd' + str(K_std) + '_xTy' + str(xTy)
    if save_data == True:
        save_file.close()
...
def get_coupling_rij(mode, nPart, phi, noise, K, xTy, seed, r_max=None):
    posFileExact = get_file_path(mode=mode, nPart=nPart, phi=phi, noise=noise, K=K, xTy=xTy, seed=seed, file_name='pos_exact')
    x, y, theta, view_time = get_pos_ex_snapshot(file=posFileExact)
...
def plot_dist_coupling_hist_diff(mode, nPart, phi, noise, K_avg, K_avg_compare, K_std, xTy, seed, bin_size=100, bin_ratio=1, r_max=None, K_max=None):

    K = str(K_avg) + "_" + str(K_std)
    K_list, rij_list = get_coupling_rij(mode=mode, nPart=nPart, phi=phi, noise=noise, K=K, xTy=xTy, seed=seed, r_max=r_max)
    K = str(K_avg_compare) + "_" + str(K_std)
    K_list_compare, rij_list_compare = get_coupling_rij(mode=mode, nPart=nPart, phi=phi, noise=noise, K=K, xTy=xTy, seed=seed, r_max=r_max)
    ## Shift to origin
...
## TO DO: add seed range
def neighbour_hist_overlay(mode, nPart, phi, noise, K_avg, K_std_range, xTy, seed, r_max, pos_ex=True, timestep_range=[1], print_stats=True):
    ...
    for K_std in K_std_range:
        ...
        # ax.hist(av_nei_i, bins=np.arange(0, np.max(av_nei_i)+1))
        unique, counts = np.unique(av_nei_i, return_counts=True)
        ax.bar(unique, counts, alpha=0.3, label=r"$K_{AVG}=$" + str(K_avg) + r"; $K_{STD}=$" + str(K_std))

    ax.set_xlabel(r"$\langle N_i\rangle$")
    ax.set_ylabel("Count")
    ax.set_title(r"$N=$" + str(nPart) + r"; $\phi=$" + str(phi) + r"; $\eta=$" + str(noise) + r"; $r_{max}=$" + str(r_max))
    ax.legend()

    folder = os.path.abspath('../plots/neighbour_hist/')
    filename = mode + '_N' + str(nPart) + '_phi' + str(phi) + '_n' + str(noise) + '_Rp1.0' + '_xTy' + str(xTy) + '_s' + str(seed) + '.png'
    if not os.path.exists(folder):
        os.makedirs(folder)
    plt.savefig(os.path.join(folder, filename))