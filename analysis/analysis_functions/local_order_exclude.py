...
def local_order_param(D, theta, r_max):
    ...
    for i in range(nPart):
        ...
        if len(idx) > 3: # Can change to any number to concentrate on collective dynamics
            theta_i = theta[idx]
            order_param.append(np.sqrt(np.sum(np.cos(theta_i))**2 + np.sum(np.sin(theta_i))**2)/len(theta_i))
...
def local_order_param_all(mode, nPart, phi, noise, K, Rp, xTy, seed_range, r_max):
    ...
    for seed in seed_range:
        ...
        o_list += local_order_param(D, theta, r_max)
    ...
    for seed in seed_range:
        ...
        for t in timestep_range:
            try:
                ...
                theta = np.array(theta)
                D = get_particle_distances(nPart, phi, xTy, x, y)
                o_all += local_order_param(D, theta, r_max)
            except:
...
def plot_local_order_hist(mode, nPart, phi, noise, K_avg, K_std, Rp, xTy, seed_range, r_max_range, pos_ex=True, timestep_range=[0]):
    ...
    folder = os.path.abspath('../plots/p_order_local_points_hist_exclude/')
